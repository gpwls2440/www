...
Datafeeds.UDFCompatibleDatafeed = function(datafeedURL, updateFrequency) {
	...
	this._barsPulseUpdater = new Datafeeds.DataPulseUpdater(this, updateFrequency || 30 * 1000);
	//this._barsPulseUpdater = new Datafeeds.DataPulseUpdater(this, updateFrequency || 0);
...
Datafeeds.UDFCompatibleDatafeed.prototype.resolveSymbol = function(symbolName, onSymbolResolvedCallback, onResolveErrorCallback) {
	...
	function onResultReady(data) {
		...
		postProcessedData["exchange-traded"] = data.exchange_traded;
		postProcessedData["exchange"] = data.exchange_listed;
		postProcessedData["exchange-listed"] = data.exchange_listed;
		postProcessedData["data_status"] = "streaming";
...
Datafeeds.UDFCompatibleDatafeed.prototype.getBars = function(symbolInfo, resolution, rangeStartDate, rangeEndDate, onDataCallback, onErrorCallback) {
    ...
    resolutionOri = resolution;
	
	if(symbolInfo.ticker.toUpperCase() != oriSimbol){
		setSymbol(symbolInfo.ticker.toUpperCase());
	}
	...
	//this._send("https://demo_feed.tradingview.com/history", {
		symbol: symbolInfo.ticker.toUpperCase(),
		//symbol: "AAPL",
		resolution: resolution,
		from: rangeStartDate,
		...
		if(firstLoading){
			...
			if(resolutionOri != "D"){
				if(bars.length > 0){
					...
					var diff = (Number(lastData.time) / 60000 ) % Number(resolutionOri);
					
					if(diff != 0){
						lastData.time = Number(lastData.time) + (60 * 1000 * (Number(resolutionOri) - diff));
					}
...
var chartBarUpdate;
var cDate;
function chartDataUpdate(r){
	
	if(resolutionOri != "D"){
		var input = r.lastTime;
		var out = input.substring(0,4) + "/" + input.substring(4,6) + "/" + input.substring(6,8) + " " + input.substring(8,10) + ":" + input.substring(10,12) + ":00";
		cDate = new Date(out).getTime();
		
		var diff = (Number(cDate) / 60000 ) % Number(resolutionOri);
		
		if(r.lastVol == 0){
			
			var lastPrice	= Number(repUnComma(lastData.close));

			cDate = cDate + (60 * 1000 * (Number(resolutionOri) - diff));
		
			if(lastData.time != cDate){
				lastData.volume = 0;
				lastData.close = lastPrice;
				lastData.open = lastPrice;
			...
			chartBarUpdate(lastData);
			
			
		} else {
			cDate = cDate + (60 * 1000 * (Number(resolutionOri) - diff));
			
			var cOpen	= Number(repUnComma(r.lastPrice));
			var cClose	= Number(repUnComma(r.lastPrice));
			var cHigh	= Number(repUnComma(r.lastPrice));
			var cLow	= Number(repUnComma(r.lastPrice));
			var cVol	= Number(repUnComma(r.lastVol));
			...
			if(lastData.time == cDate){
				...
				if(cHigh < lastData.high){
					cHigh = lastData.high;
				}
				if(cLow > lastData.low){
					cLow = lastData.low;
				}
				cVol = cVol + lastData.volume;
			}
			
			chartBarUpdate({time: cDate,close: Number(cClose),open: Number(cOpen),high: Number(cHigh),low: Number(cLow),volume: Number(cVol)});
			
			lastData.time = cDate;
			lastData.close = cClose;
			lastData.open = cOpen;
			lastData.high = cHigh;
			lastData.low = cLow;
			lastData.volume = cVol;
	
		}
	} else {
		var input = r.lastTime;
		var out = input.substring(0,4) + "/" + input.substring(4,6) + "/" + input.substring(6,8) + " 00:00:00";
		cDate = new Date(out).getTime();
